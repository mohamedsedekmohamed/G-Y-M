...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post("https://bcknd.sportego.org/api/auth/login", form);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <FaUserShield className={` ${ theme === "dark" ?"text-white":"text-maincolor"}`} />
          <span className={` ${ theme === "dark" ?"text-white":"text-maincolor"}`}>Admin Login</span>
        </motion.h2>
          ...
          <motion.button
            ...
            className={`mt-2 py-3 text-lg font-semibold rounded-lg flex items-center justify-center gap-2 transition-all duration-300 ${
              theme === "dark"
                ...
                : "bg-blue-500 hover:bg-blue-600 text-white shadow-lg shadow-blue-300/40"
            }`}
          >
